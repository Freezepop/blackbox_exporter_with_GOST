...
func ProbeHTTP(ctx context.Context, target string, module config.Module, registry *prometheus.Registry, logger *slog.Logger) (success bool) {
	...
	if httpConfig.UseHTTP3 {
		...
		// Normal endpoints stay on Go's crypto/tls. If the peer rejects that
		// handshake because it only offers GOST/legacy suites, retry once over a
		// fresh connection with the statically linked OpenSSL GOST backend.
		client.Transport, err = wrapGOSTFallback(client.Transport, httpClientConfig, logger)
		if err != nil {
			logger.Error("Error generating GOST fallback transport", "err", err)
			return false
		}
		noServerName, err = wrapGOSTFallback(noServerName, serverNamelessConfig, logger)
		if err != nil {
			logger.Error("Error generating GOST fallback transport without ServerName", "err", err)
			return false
		}
	}
	...
	if resp.TLS != nil {
		...
		registry.MustRegister(probeTLSVersion, probeTLSCipher)
		probeTLSVersion.WithLabelValues(getTLSVersion(resp.TLS)).Set(1)
		probeTLSCipher.WithLabelValues(getTLSCipher(resp.TLS)).Set(1)
		// Go's x509 parser may not understand the public-key algorithm in a GOST
		// certificate. Connectivity, TLS version and cipher remain reportable;
		// certificate metrics are exported whenever parsing succeeded.
