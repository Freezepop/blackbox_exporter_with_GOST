...
This fork additionally provides an optional, automatic GOST TLS fallback for
HTTP probes. Regular builds and all non-HTTP probers retain upstream behavior;
Linux GOST builds first use Go's standard TLS stack and retry with a statically
linked OpenSSL/gost-engine backend only after a compatible TLS handshake error.
No GOST-specific module option is required.

See [GOST support, builds and maintenance](docs/GOST.md) for architecture,
...
Release assets without a `-gost` suffix are regular upstream-compatible builds.
Linux assets with a `-gost` suffix include the automatic GOST TLS fallback.
